...
public class ViewModelKesalahan extends ViewModel {
    ...
    private void getLaporan(String nim){
        DatabaseReference daftarkesalahanRef = FirebaseDatabase.getInstance().getReference();
        daftarkesalahanRef.child("Kesalahan").orderByChild("id_pengirim").equalTo(nim).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                List<Kesalahan> list = new ArrayList<>();
                for (DataSnapshot snapshot : dataSnapshot.getChildren()){
    ...
    public MutableLiveData<List<Kesalahan>> getAllLaporan(String nim){
        if (listkesalahan == null){
            listkesalahan = new MutableLiveData<>();
            getLaporan(nim);
        }
